...
export default async function DetectionsPage({
  ...
  const exportUrl = '/api/detections/export' + (exportParams.toString() ? `?${exportParams}` : '');
  ...
  return (
    <div>
      ...
      <div className="px-8 py-7 space-y-4">
        ...
        {total === 0 ? (
          <div className="pg-card p-16 flex flex-col items-center justify-center text-center">
            <svg
              width="48"
              height="48"
              viewBox="0 0 48 48"
              fill="none"
              className="text-ink-300 mb-4"
              aria-hidden="true"
            >
            ...
            <p className="text-ink-600 font-semibold mb-1">
              {tool || severity ? 'Geen detecties voor dit filter' : 'Nog geen detecties ontvangen'}
            </p>
            <p className="text-sm text-ink-500 max-w-sm">
              {tool || severity
                ? 'Pas het filter aan om meer resultaten te zien.'
                : 'Installeer de browser-extension en open een AI-tool om je eerste detectie te registreren.'}
            </p>
            {(tool || severity) && (
              <Link href="/dashboard/detections" className="mt-4 pg-button-secondary text-sm">
                Alle filters wissen
              </Link>
            )}
          </div>
